...
def _subcharv(c: str,i: int)-> str:
    ...
    alpha = ALPHABET[index:]+ALPHABET[:index]
    try:
        return alpha[ALPHABET.index(CIPHER[i])]
    except:
        return c
...
def decipher():
    ...
    key = _subcipher('3.14159265358',CIPHER[:length])
    plain = _subkeycipher(key)
    valid = _checkvalid(plain,'3.14159265358',valid)
...
if __name__ == "__main__":
    ...
    try:
        with open ('dictionary_og.txt','r') as file:
            for line in file:
